...
function init() {
	...
	$LAYOUT->addCss('body {color:#000000; font-family:helvetica; background-color:#FFF; }');
...
/**/
	...
	if (@file_exists($SETTINGS[PSNG_SITEMAP_FILE]) && ( @filesize($SETTINGS[PSNG_SITEMAP_FILE]) > 0))
		$LAYOUT->addContentHeader('<a href="'.$SETTINGS[PSNG_SITEMAP_URL].'" target="_blank" title="View the created sitemap in a new browser window">Ver sitemap.xml </a>');
	$LAYOUT->addContentHeader('');
